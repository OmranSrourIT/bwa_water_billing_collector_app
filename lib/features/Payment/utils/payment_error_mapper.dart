class PaymentErrorMapper {
  static String getMessage(String code, String? msg) {
    switch (code) {
      ...
      case "-13":
        if (msg?.contains("116") ?? false) {
          return "تعذر إتمام عملية الدفع لعدم كفاية الرصيد في البطاقة";
        }

        if (msg?.contains("117") ?? false) {
          return "الرقم السري للبطاقة غير صحيح. يُرجى التحقق منه وإعادة المحاولة.";
        }

        if (msg?.contains("123") ?? false) {
      ...
      case "-15":
        if (msg?.contains("ADB") ?? false) {
          return "لا يمكن إتمام عملية الدفع أثناء اتصال جهاز الدفع عبر USB. يرجى فصل الاتصال والمحاولة مرة أخرى.";
        } else {
          return "انتهت المهلة الزمنية المسموح بها لإتمام عملية الدفع. يرجى إعادة المحاولة.";
        }
      case "-16":
